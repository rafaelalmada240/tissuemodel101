...
#####################################################################################################################
    for lr in L_List:
        ...
        x_down = -3
        x_up = 3
        lbound_down = np.where(vertices[Boundaries[0],1]<x_down)[0]
        bound_down = np.array(Boundaries[0])[lbound_down]
        lbound_up = np.where(vertices[Boundaries[0],1]>x_up)[0]
        ...
        while (i < M) and ((areaWound>= areaWound0/4) and (areaWound<= 4*areaWound0)):
            ...
            coords_evo_vertex[bound_up] = coords_evo_vertex[bound_up] + magF*np.ones(coords_evo_vertex[bound_up].shape)*dt
