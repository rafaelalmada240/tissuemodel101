...
for tissue_n in tissues:
    ...
    with open('tissue225_1/tissue'+str(tissue_n)+'/centers.txt','r') as text:
                for line in text:
    ...
    vertices = []
    with open('tissue225_1/tissue'+str(tissue_n)+'/vertices.txt','r') as text:
                for line in text:
                    last_line = (line.replace("\n","")).split(';')
    ...
    vorRidges = []
    with open('tissue225_1/tissue'+str(tissue_n)+'/edges.txt','r') as text:
                for line in text:
                    last_line = (line.replace("\n","")).split(';')
                    ...
                    vorRidges.append(lint4)
    Boundaries = []                
    with open('tissue225_1/tissue'+str(tissue_n)+'/boundaries.txt','r') as text:
        for line in text:
            last_line = line.replace("\n","")
    ...
    with open('tissue225_1/tissue'+str(tissue_n)+'/woundloc.txt','r') as text:
        for line in text:
    ...
    h = 5e-3*DeltaL/(2*np.sqrt(N)) #size step
    print(h)
    ...
    G_run = 1
    L_List =  [0, .5, 1.0,1.5,2.0, 2.5, 3.0, 3.5, 4.0]
    ...
    dt = mu/(K_run*np.mean(av))*5e-3 #time step bigger than K_run A0_run^2/mu
    print(dt)
    T = 5
    M = int(T/dt)
    ...
    for Lr in L_List:#range(0,int(L_run)):
        ...
        for Lw in Lw_List:
            ...
            with open('tissue225_1t/tissue'+str(tissue_n)+'/woundinfoA'+str(areaWound0.round(3))+'G'+str(G_run)+'L-'+str(Lr)+'Lw-'+str(Lw)+'Ncells'+str(N)+'.txt','a') as text:
                ...
                while (i < M) and ((areaWound>= areaWound0/2) and (areaWound<= 2*areaWound0)):
                    ...
                    text.write(str(i))
                    text.write(' ')
                    text.write(str(total_transitions))
                    text.write(' ')
                    text.write(str(perimeterWound))
                    text.write(' ')
                    text.write(str(areaWound)+ '\n')
                    # with open('tissue225_1t/tissue'+str(tissue_n)+'/edgesNcells'+str(i)+'.txt','a') as textw:
                    #     for e in vorRidges:
                    #         print(type(vorRidges))
                    #         textw.write(str(e)+ '\n')
                ...
                #plt.xlim(-6,6)
                #plt.ylim(-6,6)
                #plt.savefig('tissue225_1/tissue'+str(tissue_n)+'/AverageDisplacementsForces'+str(i)+'.png',dpi=150)
                #plt.close()
