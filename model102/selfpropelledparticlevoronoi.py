...
def energy_vor_wound(point_region,regions,ridges, vertices,vertex, K,A0,G,L,wloc, bound_wound):
...
def force_vtx_finite_grad(point_region, regions, ridges, vertices, vertex, K, A0, G, L,h):
    ...
    Ev1x = energy_vor(point_region,regions,ridges, new_vertices1x,vertex, K,A0,G,L)
    Ev2x = energy_vor(point_region,regions,ridges, new_vertices2x,vertex, K,A0,G,L)
        
    Ev1y = energy_vor(point_region,regions,ridges, new_vertices1y,vertex, K,A0,G,L)
    Ev2y = energy_vor(point_region,regions,ridges, new_vertices2y,vertex, K,A0,G,L)
...
def force_vtx_finite_grad_wound(point_region, regions, ridges, vertices, vertex, K, A0, G, L,h,wloc, bound_wound):
    ...
    Ev1x = energy_vor_wound(point_region,regions,ridges, new_vertices1x,vertex, K,A0,G,L,wloc, bound_wound)
    Ev2x = energy_vor_wound(point_region,regions,ridges, new_vertices2x,vertex, K,A0,G,L,wloc, bound_wound)
        
    Ev1y = energy_vor_wound(point_region,regions,ridges, new_vertices1y,vertex, K,A0,G,L,wloc, bound_wound)
    Ev2y = energy_vor_wound(point_region,regions,ridges, new_vertices2y,vertex, K,A0,G,L,wloc, bound_wound)
...
def force_vtx_elastic(regions,point_region, ridges, K,A0,G,L, vertices,centers, h):
    ...
    boundary_tissue = find_boundary_vertices(len(vertices),ridges)

    for v in range(LV):
        if v not in boundary_tissue:
        
            f_v = force_vtx_finite_grad(point_region, regions, ridges, vertices, v, K, A0, G, L,h)
...
def force_vtx_elastic_shear(regions,point_region, ridges, K,A0,G,L, vertices,centers, magF, h):
    ...
    for v in range(LV):
        if v not in boundary_tissue:
            ...
            if vertices[v,1] < -4:
                ...
                if vertices[v,1]>4: 
                    n1 = np.array([1,0])
                    f_v = force_vtx_finite_grad(point_region, regions, ridges, vertices, v, K, A0, G, L,h)*n1 + magF*n1
...
def force_vtx_elastic_shear_rest(regions,point_region, ridges, K,A0,G,L, vertices,centers, h):
...
def force_vtx_elastic_wound(regions,point_region, ridges, K,A0,G,L, vertices,centers, wloc,h):
    ...
    boundary_tissue = find_boundary_vertices(len(vertices),ridges)
    boundary_wound = find_wound_boundary(regions,point_region,wloc)
    ...
    for v in range(LV):
        if v not in boundary_tissue:
            if v not in boundary_wound:
                ...
                f_v = force_vtx_finite_grad_wound(point_region, regions, ridges, vertices, v, K, A0, G, L,h,wloc, boundary_wound)
            NeighR, NeighC = find_vertex_neighbour_centers(regions,point_region,v)
            for i in range(len(NeighC)):
                for j in range(i+1,len(NeighC)):
                    ci = centers[NeighC[i]]
