...
for i in range(N):
    av.append(sppv.area_vor(vorPointRegion,vorRegions,np.array(sppv.vertices_in_bound(list(vor.vertices),5)),i))
...
coords_evo[:,:,0] = coords
coords_evo_vertex[:,:,0] = np.array(sppv.vertices_in_bound(list(vor.vertices),L_max))
...
if continue_option == 1:
    ...
    if wound_option == 1:
        ...
        for i in range(M-1):
            #Compute forces
            F_vertex = sppv.force_vtx_elastic_wound(vorRegions, vorPointRegion, vorRidges_array[:,:,i], K_run,A0_run,G_run,L_run,coords_evo_vertex[:,:,i],coords_evo[:,:,i],wloc,h)
            ...
            with open('woundareaG'+str(G_run)+'L'+str(L_run)+'.txt','a') as text:
                text.write(str(i))
                text.write(' ')
            ...
            #vorRidges_array[:,:,i+1], coords_evo_vertex[:,:,i+1], vorRegions, transition_counter =  tpt.T1transition(np.array(coords_evo_vertex[:,:,i+1]),np.array(vorRidges_array[:,:,i]),vorRegions, vorPointRegion,0.1*r0)
        ...
        if shear_option ==1: 
            ...
            #Initial relaxation
            vorVertices = np.array(sppv.vertices_in_bound(list(vor.vertices),L_max))
            N_relax = 500
            for i in range(N_relax-1):
                #Compute forces
                F_vertex = sppv.force_vtx_elastic_shear_rest(vorRegions, vorPointRegion, vorRidges, K_run,A0_run,G_run,L_run,vorVertices,coords,h)
                #Force_vertex_vector[:,:,i] = F_vertex
                ...
                #Do topological transition T1 (hopefully works)
                #vorRidges, vorVertices, vorRegions, transition_counter =  tpt.T1transition(np.array(vorVertices),np.array(vorRidges),vorRegions, vorPointRegion,0.1*r0)
                
            coords_evo[:,:,0] = coords
            coords_evo_vertex[:,:,0] = np.array(sppv.vertices_in_bound(list(vorVertices),L_max))
            vorRidges_array[:,:,0] = vorRidges
            vorRegions_array = []
            vorRegions_array.append(vorRegions)
            ...
            for i in range(M-1):
                #Compute forces
                F_vertex = sppv.force_vtx_elastic_shear(vorRegions, vorPointRegion, vorRidges_array[:,:,i], K_run,A0_run,G_run,L_run,coords_evo_vertex[:,:,i],coords_evo[:,:,i],magF,h)
                Force_vertex_vector[:,:,i] = F_vertex
            ...
            for i in range(M-1):
                #Compute forces
                F_vertex = sppv.force_vtx_elastic_shear_rest(vorRegions, vorPointRegion, vorRidges_array[:,:,i], K_run,A0_run,G_run,L_run,coords_evo_vertex[:,:,i],coords_evo[:,:,i],h)
                Force_vertex_vector[:,:,i] = F_vertex
