        ...
        if np.isnan(np.sum(vertices[i])):
            ...
            if len(list_neigh_v_not_excluded) > 2:
                ...
                if (len(vertices_neigh_min) > 2) and (len(list(set(list_neigh_v_not_excluded).intersection(vertices_neigh_min)))<1):
                    ...
                    #For vertex i
                    regions_neigh_v, center = sppv.find_vertex_neighbour_centers(regions,point_region,i)
                                
                    #For neighbouring vertex v_min
                    regions_neigh_vmin, center = sppv.find_vertex_neighbour_centers(regions,point_region,v_min)
                    ...
                    if ((len(region_exc_v)>0) and (len(region_exc_vmin)>0)) and (len(region_common)>1): 
                        if lv_min < thresh_len: 
                            ...
                            regions[region_exc_vmin[0]].append(v_min)
                            regions[region_exc_v[0]].append(i)
                            
                        
                            regions[new_region_exc_v].remove(v_min)
                            regions[new_region_exc_min].remove(i)
                        
                            new_neighv = list(set(regions[new_region_exc_v]).intersection(regions[region_exc_vmin[0]]))
                            new_neighvm = list(set(regions[region_exc_v[0]]).intersection(regions[new_region_exc_min]))
                            ...
                            vertices_neigh = list(set(list_neigh_v_not_excluded).difference(new_neighvm).union(new_neighv))
...
def T1transition_old(vertices, ridges, regions, point_region,thresh_len):
